package cn.codercheng.elasticsearchspringbootdemo.service.impl;


import cn.codercheng.elasticsearchspringbootdemo.service.PositionService;
import cn.codercheng.elasticsearchspringbootdemo.util.DBHelper;
...
public class PositionServiceImpl implements PositionService {
    ...
    public List<Map<String, Object>> searchPos(String keyword, int pageNo, int pageSize) throws IOException {
        ...
        // 搜索
        SearchHit[] searchHits = searchByType("POSITION_NAME", keyword, pageNo, pageSize);
        for (SearchHit hit : searchHits) {
            list.add(hit.getSourceAsMap());
        }

        //检索到的职位信息不够5条 则需要启用positionAdvantage 查找 美女多、员工福利好 的企业职位信息进行补充够5条
        if (list.size() <5) {
            searchHits = searchByType("", "", pageNo, 5-list.size());
            for (SearchHit hit : searchHits) {
                list.add(hit.getSourceAsMap());
            }
        }
    ...
    public SearchHit[] searchByType(String typeName, String keyword, int pageNo, int pageSize) throws IOException {
        ...
        if ("POSITION_NAME".equals(typeName)) {
            builder = QueryBuilders.matchQuery("positionName", keyword);
        } else {
            builder = QueryBuilders.matchQuery("positionAdvantage", "美女多、员工福利好");
        }
