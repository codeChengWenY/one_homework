...
public class LgDispatcherServlet extends HttpServlet {
    ...
    // auth权限
    private Map<String, Object> auth = new HashMap<String, Object>();  // 存储方法有的权限
    ...
    private void initHandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            if (aClass.isAnnotationPresent(Security.class)) {
                ...
                baseAuth = annotation.value(); // 等同于/'张三','李四'
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                // 如果标识，就处理
                Security authAnnotation = method.getAnnotation(Security.class);
                if (Objects.nonNull(authAnnotation)) {
                    String[] security = authAnnotation.value();  // /auth
                    if (baseAuth.length > 0) {
                        for (int j = 0; j < baseAuth.length; j++) {
                            String bathAuth = baseAuth[j];
                            for (int i1 = 0; i1 < security.length; i1++) {
                                String methodAuth = security[i1];
                                authList.add(bathAuth + methodAuth);
                            }
                        }
                    } else {
                        for (int i1 = 0; i1 < security.length; i1++) {
                            String methodAuth = security[i1];
                            authList.add(methodAuth);
                        }
                    }
                } else {
                    if (baseAuth.length > 0) {
                        for (int j = 0; j < baseAuth.length; j++) {
                            String bathAuth = baseAuth[j];
                            authList.add(bathAuth);
                        }
                    }
                }
                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(), method, Pattern.compile(url), authList);
...
//        method.invoke() //
        ...
        try {
            // 判断是否有权限
            String reqUserNameParameter = req.getParameter("username");
            String username = StringUtils.isBlank(reqUserNameParameter) ? "" : reqUserNameParameter;
            username = new String(username.getBytes("ISO-8859-1"), "UTF-8");
            if (0 != handler.getAuthList().size()) {
                if (!handler.getAuthList().contains(username)) {
                    resp.setContentType("text/plain; charset=utf-8");
                    resp.getWriter().write("没有调用此接口权限");
                    return;
                }
            }
            Object invoke = handler.getMethod().invoke(handler.getController(), paraValues);
            resp.setContentType("text/plain; charset=utf-8");
            resp.getWriter().write(Objects.isNull(invoke) ? "" : invoke.toString());
        } catch (IllegalAccessException e) {
            e.printStackTrace();
